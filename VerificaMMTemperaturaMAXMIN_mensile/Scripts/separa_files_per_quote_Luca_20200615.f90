PROGRAM separa_files_per_quote_Luca_20200615
! Codice per scrivere separare un file con le temperature in tre files a seconda della classe di quota   
! 2018 - Paolo  

!Modifica Luca: inserito terzo parametro di ingresso cartella di salvataggio
...
IMPLICIT NONE 
 ...
 CHARACTER(100)      :: filein,fileanag,save_path 
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
...
!################################################################################################################  
      ...
      OPEN(unit=12,file=trim(save_path)//"/fileout700.txt",status='unknown',iostat=ios)  
      if (ios/=0) then 
        print *,'ERRORE in apertura fileout700 '     
        stop 
      endif    

      OPEN(unit=13,file=trim(save_path)//"/fileout1500.txt",status='unknown',iostat=ios)  
      if (ios/=0) then 
        print *,'ERRORE in apertura fileout1500 '     
        stop 
      endif  
      
      OPEN(unit=14,file=trim(save_path)//"/fileout3000.txt",status='unknown',iostat=ios)  
      if (ios/=0) then 
...
END PROGRAM separa_files_per_quote_Luca_20200615
