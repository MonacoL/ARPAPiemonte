...
nscadenze_ieri=4
nscadenze_ieri_psa=2
nscadenze_ieri_psa_missing=2
nmodels_plus_obs=nmodels
nosservati=1
ndati=nmodels*2+1
ngiorni_backwards=2 #quanti giorni previsionali vado indietro rispetto a oggi

aree=c("A","B","C","D","E","F","G","H","I","L","M","T","V")
modelli_labels=c("MOLOCH","BOLAM","COSMO-2I","COSMO-I5","ECMWF","ICONEU","WRF", "PSA")
modelli_path=c("m0103","b0700","c2200","c5m00","e0100","i0700","wrf00","prvsa")
modelli_values=c("01-m0103","02-b0700","03-c2200","04-c5m00","05-e0100","06-i0700","07-wrf00","08-prvsa")
...
scadenze_ieri_psa=c("12-18","18-24")
scadenze_ieri_psa_missing=c("00-06","06-12")
#scadenze_ieri=c("12-18","18-24")
#scadenze_ieri_missing=c("00-06","06-12")
col_scadenze_oss=c(3,4,5,6)
col_scadenze_mod_ieri=c(1,2,3,4)
col_scadenze_mod_ieri_psa=c(1,2)
#col_scadenze_mod_ieri=c(1,2)
col_scadenze_mod_altroieri=c(5,6,7,8)
col_scadenze_mod_altroieri_psa=c(3,4,5,6)
dati=matrix(, nrow =0, ncol = ncol)
colnames(dati)<-c("Precipitazione","Scadenza","Area","Modello","Giorno")

fileossaltroieri<-paste(work_path,"/","osser_06hh_",altroieri,"12_",tipo,".bal",sep="")
...
for (m in seq(1,nmodels)){
    prima_scadenza="00"
    if (modelli_path[m]=="prvsa"){
        prima_scadenza="12"
    }
    filemodieri<-paste(work_path,"/",modelli_path[m],"_06hh_",ieri,prima_scadenza,"_",tipo,".bal",sep="")
    modieri<-read.table(filemodieri,header=F,na.strings="-98")
    filemodaltroieri<-paste(work_path,"/",modelli_path[m],"_06hh_",altroieri,prima_scadenza,"_",tipo,".bal",sep="")
    ...
    for (s in seq(1,nscadenze_altroieri)){
        ...
        for (a in seq(1,naree)){
            if (modelli_path[m]=="prvsa"){
                riga_previsioni[a,"Precipitazione"]=modaltroieri[a,col_scadenze_mod_altroieri_psa[s]]
            }else{
    ...
    nscad=nscadenze_ieri
    scadenze=scadenze_ieri
    colonne=col_scadenze_mod_ieri
    if (modelli_path[m]=="prvsa"){
        for (s in seq(1,nscadenze_ieri_psa_missing)){
            riga_previsioni=matrix(, nrow = naree, ncol = ncol)
            colnames(riga_previsioni)<-c("Precipitazione","Scadenza","Area","Modello","Giorno")  
            for (a in seq(1,naree)){
                riga_previsioni[a,"Precipitazione"]=NA
                riga_previsioni[a,"Scadenza"]=scadenze_ieri_psa_missing[s]
                riga_previsioni[a,"Area"]=aree[a]
                riga_previsioni[a,"Modello"]=modelli_values[m]
        ...
        nscad=nscadenze_ieri_psa
        scadenze=scadenze_ieri_psa
        colonne=col_scadenze_mod_ieri_psa
    }

    for (s in seq(1,nscad)){
        riga_previsioni=matrix(, nrow = naree, ncol = ncol)
        colnames(riga_previsioni)<-c("Precipitazione","Scadenza","Area","Modello","Giorno")  
        for (a in seq(1,naree)){
            riga_previsioni[a,"Precipitazione"]=modieri[a,colonne[s]]
            riga_previsioni[a,"Scadenza"]=scadenze[s]
            riga_previsioni[a,"Area"]=aree[a]
            riga_previsioni[a,"Modello"]=modelli_values[m]
            riga_previsioni[a,"Giorno"]=label_run_ieri
        }
...
for (a in seq(1,naree)){
    for (s in seq(1,nscadenze_altroieri)){
        dato_oss <- ossaltroieri[a,col_scadenze_oss[s]]
...
if (tipo=="max"){
    ...
    for (a in seq(1,naree)){
        massimo=max(ossaltroieri[a,3:6],na.rm=T)
    ...
    fileConn<-file(paste(coloritxt_path,"/qpf_aree_colori.txt",sep=""))
